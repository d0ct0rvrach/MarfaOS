...
void set_idt_gate(int n, unsigned int handler) {
    ...
    idt_entries[n].sel       = 0x08; // kernel segment
...
void init_idt() {
    ...
    for (int i = 0; i < 256; i++) set_idt_gate(i, 0); // clear the table

    set_idt_gate(32, (unsigned int)irq0); // timer
    set_idt_gate(33, (unsigned int)irq1); // keyboard

    extern void isr0();
    set_idt_gate(0, (unsigned int)isr0); // division by zero, see shell.c - crash
